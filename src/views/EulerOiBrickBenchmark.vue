  ...
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900">
    ...
    <div class="p-4 sm:p-6 mx-auto max-w-5xl">
      <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-2">
        单边欧拉 · OI 加权（1m 点砖 + 5m metrics）
      </h1>
      <p class="text-sm text-gray-600 dark:text-gray-400 mb-4 space-y-1">
        <span class="block">
          砖石线由本地 <code class="px-1 rounded bg-gray-200 dark:bg-gray-800">*_1m.jbar</code> 按所选基准时钟聚合（可选<strong>动态 ATR 快照锁</strong>：每根新砖诞生时读取 Wilder ATR，本砖阈值 <code>S = ATR × λ</code> 锁死至切砖，等价 Lopez de Prado 信息条 —— 以波动率归一化砖宽）；<strong>Q</strong>=主腿砖收益 Z，
      ...
      <section class="mb-6 bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
        <h2 class="text-lg font-semibold mb-3">参数与构建</h2>
        <div class="grid grid-cols-1 sm:grid-cols-4 gap-4">
          <div class="sm:col-span-2">
            <label class="block text-sm text-gray-700 dark:text-gray-200 mb-1">切砖模式</label>
            <select
              v-model="form.brickMode"
              class="w-full px-3 py-2 border rounded-md bg-white dark:bg-gray-800 text-sm"
            >
              <option value="percent">固定砖幅 %（写盘 jbar / 与后端一致）</option>
              <option value="atr_dynamic">动态 ATR 快照锁（浏览器内从 1m 生成，不落盘）</option>
            </select>
          </div>
          <div>
          ...
          <div v-if="form.brickMode === 'atr_dynamic'">
            <label class="block text-sm text-gray-700 dark:text-gray-200 mb-1">ATR 周期 P（Wilder）</label>
            <input v-model.number="form.atrPeriod" type="number" min="2" max="200" step="1" class="w-full px-3 py-2 border rounded-md bg-white dark:bg-gray-800" />
          </div>
          <div v-if="form.brickMode === 'atr_dynamic'">
            <label class="block text-sm text-gray-700 dark:text-gray-200 mb-1">阈值 λ（S = ATR×λ）</label>
            <input v-model.number="form.atrBrickMult" type="number" min="0.05" max="5" step="0.05" class="w-full px-3 py-2 border rounded-md bg-white dark:bg-gray-800" />
          </div>
          <div v-if="form.brickMode === 'atr_dynamic'">
            <label class="block text-sm text-gray-700 dark:text-gray-200 mb-1">ATR 未就绪时回退砖幅 %</label>
            <input v-model.number="form.atrFallbackMovePercent" type="number" min="0.01" max="10" step="0.05" class="w-full px-3 py-2 border rounded-md bg-white dark:bg-gray-800" />
          </div>
          <div>
      ...
      <section v-if="dashboardRows.length && benchmarkBars.length" class="mb-6 bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
        ...
        <RenkoFourPaneDashboard
          dashboard-heading="单边 OI 加权欧拉 · 四维盘"
          :rows="dashboardRows"
          ...
          :custom-euler-points="eulerOiCustomPoints"
        />
        <div
          v-if="eulerOiCustomPoints.length"
        ...
        >
          ...
          <template v-if="eulerOiBacktestResult">
            <p class="text-xs text-gray-500 dark:text-gray-400 mb-1">
              信号与主腿收价均在当根收盘确定（不向未来取数）；合计为各笔<strong>净</strong>收益率 % 算术相加，非复利净值。选「静风+反向熔断」时：多遇<strong>红3</strong>、空遇<strong>绿1</strong>即平；紫/黄不单独触发平仓。
            </p>
        ...
        <p v-if="eulerOiCustomPoints.length === 0 && bars.length" class="mt-2 text-xs text-amber-700 dark:text-amber-300">
...
import { buildBtcPointBrickKline, fetchBtcPointBrickKlineBars, fetchBinanceHistoryKlineBars, getBinanceTopVolumeContracts } from '@/api'
import {
  toCandles,
  ...
  buildBtcMasterClockAtrSnapshotLockPointBricks,
...
const eulerOiCustomPoints = computed(() => {
  const rows = dashboardRows.value
...
const eulerOiBacktestResult = computed(() => {
  const pts = eulerOiCustomPoints.value
  const rows = dashboardRows.value
...
async function doBuild() {
  ...
  if (form.value.brickMode === 'atr_dynamic') {
    error.value =
      '当前为「动态 ATR 快照锁」模式：砖序列在浏览器内由 1m 生成，无需服务端写盘。请直接点「加载砖 + metrics + 绘图」。若需生成固定% 的 *_ABOPOINTBRICK.jbar，请切回「固定砖幅%」后再点重构。'
    return
  }
...
async function doLoadBars() {
  ...
  try {
    ...
    if (form.value.brickMode === 'atr_dynamic') {
      ...
      if (!bench1m.length) {
        error.value = '未加载到基准合约 1m K 线，无法计算 ATR 动态砖'
        return
      }
      const syms =
        symbolUpper.value === benchmarkUpper.value
          ? [symbolUpper.value]
          : [symbolUpper.value, benchmarkUpper.value]
      const seriesBySymbol = {}
      if (symbolUpper.value === benchmarkUpper.value) {
        seriesBySymbol[symbolUpper.value] = bench1m
      } else {
        seriesBySymbol[symbolUpper.value] = main1m
        seriesBySymbol[benchmarkUpper.value] = bench1m
      }
      const built = buildBtcMasterClockAtrSnapshotLockPointBricks({
        masterOneM: bench1m,
        seriesBySymbol,
        symbols: syms,
        masterSymbol: benchmarkUpper.value,
        atrPeriod: Math.max(2, Math.min(200, Math.floor(Number(form.value.atrPeriod)) || 14)),
        atrMult: Number.isFinite(Number(form.value.atrBrickMult)) ? Number(form.value.atrBrickMult) : 0.5,
        fallbackMovePercent: Number.isFinite(Number(form.value.atrFallbackMovePercent))
          ? Number(form.value.atrFallbackMovePercent)
          : 0.5,
      })
      const mainBricks = built[symbolUpper.value] || []
      const benchBricks = built[benchmarkUpper.value] || []
      meta.value = {
        count: mainBricks.length,
        jbarPath: `client:atr_snapshot_lock(P=${form.value.atrPeriod},λ=${form.value.atrBrickMult})`,
      }
      ...
      const [data, benchData, metricsData, oneMData] = await Promise.all([
      ...
      bars.value = normalizeBrickBars(data?.bars)
      benchmarkBars.value = normalizeBrickBars(
        symbolUpper.value === benchmarkUpper.value ? data?.bars : benchData?.bars
      )
      metricsBars.value = Array.isArray(metricsData?.bars) ? metricsData.bars : []
      oneMinBarsRaw.value = Array.isArray(oneMData?.bars) ? oneMData.bars : []
    }
    if (!metricsBars.value.length && !error.value) {
